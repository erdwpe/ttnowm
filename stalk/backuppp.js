...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Coc
    document.getElementById('coc-id').addEventListener('keydown', e => {
      if (e.key === 'Enter') stalkCoC();
    });
...
document.querySelectorAll('.tab-btn').forEach(btn => {
  ...
  // Clash of Clans Stalk
  async function stalkCoC() {
    const playerId = document.getElementById("coc-id").value.trim().replace("#", "%23");
    const resultBox = document.getElementById("coc-result");
    if (!playerId) return resultBox.textContent = "Masukkan tag pemain (misalnya: #ABC123)";
  
    const url = `https://erdwpe.it.com/proxy/player?id=${playerId}`;
    ...
    try {
      ...
      if (!result.name) throw new Error("Data tidak ditemukan");
  
      // Ambil icon-icon label
      const labels = result.labels?.map(label => {
        const icon = label.iconUrls?.small || label.iconUrls?.medium || "";
        return icon ? `<img src="${icon}" alt="${label.name}" class="label-icon" title="${label.name}" />` : "";
      }).join(" ") || "-";
      ...
      const html = `
        ...
        <ul>
          ...
          <li><strong>Tag:</strong> ${result.tag}</li>
          <li><strong>Level:</strong> ${result.expLevel}</li>
          <li><strong>Trofi Tertinggi:</strong> ${result.bestTrophies}</li>
          <li><strong>War Stars:</strong> ${result.warStars}</li>
          <li><strong>Town Hall:</strong> ${result.townHallLevel}</li>
          <li><strong>Builder Hall:</strong> ${result.builderHallLevel ?? "-"}</li>
          <li><strong>Clan:</strong> ${result.clan?.name ?? "-"}</li>
          <li><strong>Clan Level:</strong> ${result.clan?.clanLevel ?? "-"}</li>
          <li><strong>Donasi:</strong> ${result.donations}</li>
          <li><strong>Donasi Diterima:</strong> ${result.donationsReceived}</li>
          <li><strong>Label:</strong><br/> ${labels}</li>
        </ul>
        <li><strong>Trofi:</strong> ${result.trophies}</li>
        ${result.league?.iconUrls?.small
          ? `<img src="${result.league.iconUrls.small}" alt="League Icon" />`
          : result.badgeUrls?.small
          ? `<img src="${result.badgeUrls.small}" alt="Badge Icon" />`
          : ""
        }
      `;
